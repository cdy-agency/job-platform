...
import { CheckCircle, XCircle, Users, Building, FileCheck } from "lucide-react";
...
export default function AdminDashboardPage() {
  ...
  useEffect(() => {
    const getRecentEmployee = async () => {
      try {
        ...
        console.log("the fetched employee", res);
  ...
  const approvedCompanies = companiesArray.filter(
    (c) => c.isApproved === true
  ).length;
  const pendingCompanies = companiesArray.filter(
    (c) => c.isApproved !== true
  ).length;
          ...
          <tbody>
            {employeesArray.map((emp) => (
              <tr key={emp._id} className="hover:bg-gray-50">
                ...
                <td className="border border-gray-300 p-2">{emp.location}</td>
              </tr>
            ))}
          </tbody>
        </table>
